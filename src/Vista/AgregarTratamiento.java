...
public class AgregarTratamiento extends javax.swing.JInternalFrame {
    ...
    private String obtenerTipoSeleccionado() {
        if (radioFacial.isSelected()) return "Facial";
        if (radioCorporal.isSelected()) return "Corporal";
        if (radioRelajacion.isSelected()) return "Relajacion";
        if (radioEstetico.isSelected()) return "Estetico";
        return "";
    }
    ...
    private int generarCodigoTratamiento() {
        return (int) (System.currentTimeMillis() % 100000); // Temporal
    }
    ...
    private void btnAgregarNuevoTratamActionPerformed(java.awt.event.ActionEvent evt) {                                                      
        ...
        try {
            ...
            // Generar codigo
            int codTratam = generarCodigoTratamiento();
            
            // Crear nuevo tratamiento (activo por defecto)
            Tratamiento nuevoTratamiento = new Tratamiento(
                codTratam, nombre, detalle, tipo, duracion, costo, true, producto
            );
            ...
            JOptionPane.showMessageDialog(this, "Error en formato numérico: Duración y Costo deben ser números válidos");
        } catch (Exception e) {
